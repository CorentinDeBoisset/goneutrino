<template>
  <form @submit.prevent="" class="name-selector">
    <div class="name-selector__input-block">
      <div class="name-selector__input-label">{{ $t("hello") }}</div>
      <div class="name-selector__input-label">{{ $t("pick-nickname") }}</div>
      <input v-model="nickname" :placeholder="$t('nickname-placeholder')" />
    </div>
    <div class="name-selector__submit-block">
      <button class="btn" type="submit">{{ $t("submit-nickname") }} →</button>
    </div>
